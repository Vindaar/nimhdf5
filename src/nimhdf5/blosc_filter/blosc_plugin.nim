...
proc bloscSetLocal(dcpl, dtype, space: hid_t): herr_t =
  ...
  var
    ...
    flags: cuint
    nelements = 8.csize_t
    ## TODO: this should be `cuint`, but `cuint` is an alias for `uint32`
    ## ref: https://github.com/nim-lang/Nim/issues/16246
    ## revert once above is fixed
    values = newSeq[uint32](8)
...
let available = H5Zfilter_avail(FILTER_BLOSC) == 1
if not available:
  var
    version: string
    date: string
